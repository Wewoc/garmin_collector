...
> *"Architecture and logic from the human. Python implementation from the AI."*
> — Le Chat (Mistral), cross-model review

This table documents the origin of key decisions, features, and working methods in the project. Based on exported chat logs from Claude.ai covering approximately v0.4 through v1.3.1.

---

| Category | Idea / Decision | User influence | Claude.ai influence |
|---|---|---|---|
...
| **User** | Bulk-first over API-first (v1.3.0) | Overruled Claude's assessment with concrete experience: API blocked for a week via 429 | Had rated bulk as operationally impractical |
...
| **User corrects Claude** | Export range vs. sync range in UI | *"The range in the interface is only for sync, not for the scripts."* | Had conflated two separate UI concepts |
| **User corrects Claude** | Bulk-first vs. API-first | Concrete counter-argument: *"429 for over a week"* | Had rated bulk as impractical without knowing the operational context |
| **User corrects Claude** | Two collector scripts vs. one with config block | *"Scripts for purists — they won't have Python anyway"* | Had proposed two separate files |
| **Claude brakes User** | No SSD backup in the application | *"That's OS responsibility, not application responsibility"* | User: *"Ok, I was thinking too far."* |
| **Claude brakes User** | No file split for `quality_log.json` | — | Argued against splitting despite growing file size |
| **Claude brakes User** | No implementation without explicit instruction | Self-correction documented in chat, stored as memory rule | User: *"nothing yet"* as workaround — acknowledged as his compensation, not his job |
